...
export function EditPartyDialog({ open, party, onClose, onSuccess }: EditPartyDialogProps) {
  const { t } = useTranslation(['parties', 'common']);
  const [loading, setLoading] = useState(false);
  const { errors, clearError, validate } = useFormErrors<'name' | 'phone' | 'email'>();

  // ── Sections open/closed (auto-open if party has data) ──────────────────
  ...
  // ── Populate state from party on open ────────────────────────────────────
  useEffect(() => {
    if (open && party) {
      setForm({
        name: party.name || '',
        ...
        email: party.email || '',
        type: (party.type as 'SUPPLIER' | 'CUSTOMER' | 'BOTH') || 'BOTH',
        is_mine: party.is_mine ?? false,
      });
      setGst({
        registrationType: party.gst_registration_type || 'UNREGISTERED',
        gstin: party.gst_number || '',
        gstState: party.gst_state || '',
      ...
      setAddressQuery(party.address || '');
      setAddressSuggestions([]);
      setShowSuggestions(false);
      setAddressNoResults(false);
      setCredit({
        periodDays: party.credit_period_days != null ? String(party.credit_period_days) : '',
        limit: party.credit_limit != null ? String(party.credit_limit) : '',
      });
      const ob = Number(party.opening_balance ?? 0);
      // Backend sign convention: positive = payable, negative = receivable.
      setOpeningBal({
        amount: ob !== 0 ? String(Math.abs(ob)) : '',
        type: ob > 0 ? 'payable' : 'receivable',
      });
      setContacts(
      ...
      const hasGst = !!(party.gst_number || party.gst_state || party.pan_number || (party.gst_registration_type && party.gst_registration_type !== 'UNREGISTERED'));
      const hasAddr = !!(party.address || party.city || party.state || party.pincode);
      const hasCredit = !!(party.credit_period_days || party.credit_limit);
      const hasContacts = !!(party.contacts && party.contacts.length > 0);
      const hasBanks = !!(party.bank_accounts && party.bank_accounts.length > 0);
      setSections({
      ...
      // Load pincodes if city is set
      if (party.city) loadPincodes(party.city);
      else setPincodes([]);
    }
  }, [open, party, loadPincodes]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true);
      await profileApi.updateParty(party.id, {
        name: form.name.trim(),
        phone: form.phone,
        // Cleared optional fields must be sent as null, not undefined —
        // JSON.stringify drops undefined keys and the service only writes
        // keys that are `!== undefined`, so a cleared field kept its old
        // value forever.
        whatsapp: form.whatsapp.trim() || null,
        // Never rewrite a CUTTER mirror-party's type — it would surface the
        // cutter as a duplicate normal party in the Parties list.
        ...((party.type as string) !== 'CUTTER' && { type: form.type }),
        email: form.email.trim() || null,
        isMine: form.is_mine,

        // GST
        gstRegistrationType: gst.registrationType,
        gstNumber: gst.gstin.trim() || null,
        gstState: gst.gstState || null,
        panNumber: gst.panNumber.trim() || null,

        // Address
        address: addr.address.trim() || null,
        city: addr.city || null,
        state: addr.state || null,
        pincode: addr.pincode ? addr.pincode.split('—')[0].trim() : null,

        // Credit
        creditPeriodDays: credit.periodDays ? parseInt(credit.periodDays) : null,
        creditLimit: credit.limit ? parseFloat(credit.limit) : null,

        // Opening Balance — backend sign convention: positive = payable
        // (we owe them), negative = receivable (they owe us).
        openingBalance: openingBal.amount
          ? (openingBal.type === 'payable'
              ? Math.abs(parseFloat(openingBal.amount))
              : -Math.abs(parseFloat(openingBal.amount)))
          : 0,

        // Contacts / Banking — the sections are editable in this dialog, so
        // their state must actually be sent (it used to be silently dropped).
        contacts: contacts
      ...
      toast.success(t('parties:party_updated'));
      onSuccess?.();
    } catch (err: any) {
      toast.error(err.response?.data?.message || t('parties:party_update_error'));
    } finally {
      setLoading(false);
    }
  };
      ...
      <DialogContent className="sm:max-w-lg max-h-[90vh] flex flex-col p-0 gap-0">
        <DialogHeader className="px-6 pt-6 pb-4 border-b border-border shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <DialogTitle>{t('parties:edit_party_title')}</DialogTitle>
              <DialogDescription>{t('parties:edit_party_desc')}</DialogDescription>
            </div>
          </div>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col flex-1 min-h-0">
            ...
            <div className="rounded-lg border border-border bg-muted/30 p-3 space-y-2.5">
              ...
              <div className="flex gap-2">
                <div className="relative flex-1">
                  ...
                  <input
                    ...
                    min={0}
                    placeholder="0.00"
                    value={openingBal.amount}
                    onChange={e => setOpeningBal(p => ({ ...p, amount: e.target.value }))}
                ...
                <div className="flex rounded-md border border-input overflow-hidden shrink-0">
                  {(['receivable', 'payable'] as const).map(opt => (
                    <button
                      key={opt}
                      type="button"
                      onClick={() => setOpeningBal(p => ({ ...p, type: opt }))}
                      className={[
                        'px-3 py-2 text-xs font-medium transition-colors',
                        openingBal.type === opt
                          ? opt === 'receivable' ? 'bg-green-500 text-white' : 'bg-red-500 text-white'
              ...
              {openingBal.amount && (
                <p className="text-xs text-muted-foreground">
                  {openingBal.type === 'receivable'
                    ? t('parties:ob_receivable_hint', { amount: `₹${openingBal.amount}` })
                    : t('parties:ob_payable_hint', { amount: `₹${openingBal.amount}` })}
                </p>
              )}
            </div>
          ...
          <DialogFooter className="px-6 py-4 border-t border-border shrink-0 bg-background">
            ...
            <Button type="submit" loading={loading}>{t('parties:save_party_btn')}</Button>
          </DialogFooter>
        </form>
      </DialogContent>
    </Dialog>
